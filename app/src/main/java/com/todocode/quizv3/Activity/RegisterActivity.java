...
import com.todocode.quizv3.Util.AppUtil;
...
public class RegisterActivity extends AppCompatActivity {
    ...
    int RC_SIGN_IN = 0;
    public SharedPreferences emailVerificationOption;
        ...
        register.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (validator.validate()) {
                    Dialog dialog = AppUtil.showProgress(RegisterActivity.this);
                    registerUser();
                    validator.clear();
                    dialog.dismiss();
                }
    ...
    public void setupRules() {
        ...
        validator.addValidation(this, R.id.register_password, "^(?=.*[A-Za-z])(?=.*\\d)(?=.*[@$!%*#?&])[A-Za-z\\d@$!%*#?&]{8,}$", R.string.password_requirement_error);
    }
    ...
    private void registerUserIfNoEmailVerification(final String enteredName, final String enteredEmail, final String enteredPassword) {
        StringRequest stringRequest = new StringRequest(Request.Method.POST, url+"/api/players/new", new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                try {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, url+"/api/players/new/verify", new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                try {
                    ...
                    if (success.equals("1")){
                        Log.e("success", "1");
                        if (emailVerificationOption.getString("emailVerificationOption", "").equals("yes")) {
                            Log.e("email", "yes");
                            mProgressBar.setVisibility(View.GONE);
                            ...
                            startActivity(main);
                            finish();
                        } else if(emailVerificationOption.getString("emailVerificationOption", "").equals("no")) {
                            Log.e("email", "no");
                            mProgressBar.setVisibility(View.GONE);
                            registerUserIfNoEmailVerification(enteredName, enteredEmail, enteredPassword);
                        }
                        // IF NO
                    }
                    // Name Exists
